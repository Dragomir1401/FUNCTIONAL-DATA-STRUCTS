...
insertTree tree1 (tree2 : rest) = case (tree1, tree2) of 
    (EmptyTree, _) -> tree2 : rest -- emptyTree si orice
    (_ , EmptyTree) -> tree1 : rest -- orice si emptytree
    (Node p1 k1 children1, Node p2 k2 children2) | p1 < p2 -> tree1 : tree2 : rest | otherwise -> EmptyTree : insertTree (attach tree1 tree2) rest
...
insert prio key (BinomialHeap size trees) = BinomialHeap (size + 1) (insertTree (Node prio key []) trees)
...
findMin (BinomialHeap size trees) =
  case trees of
    [] -> Nothing
    _ -> Just (minimumBy (compare `on` getPriority) roots)
  where
    roots = [root | Node prio key children <- trees, let root = (prio, key)]
