...
-}
insertTree :: (Ord p, Eq k)  => BinomialTree p k -> [BinomialTree p k] -> [BinomialTree p k]
insertTree tree [] = [tree]
insertTree currTree (trees : rest) = case (currTree, trees) of
    (_, EmptyTree) -> currTree : rest
    (Node p1 k1 children1, Node p2 k2 children2) -> EmptyTree : insertTree (attach currTree trees) rest
...
-}
insert :: (Ord p, Eq k)  => p -> k -> BinomialHeap p k -> BinomialHeap p k
...
findMin t@(BinomialHeap 0 _) = Nothing 
...
-}
mergeTrees :: (Ord p, Eq k) => [BinomialTree p k] -> [BinomialTree p k] -> [BinomialTree p k]
mergeTrees [] [] = []
mergeTrees trees1 trees2 = 
    -- `res` is a pair of the resulting tree and the accumulated remainder
    let res = mapAccumL mergeAux EmptyTree (zipExtend EmptyTree EmptyTree trees1 trees2)
        mergeAux acc (tree1, tree2) = 
            case (acc, tree1, tree2) of
                -- If all trees are empty, return two empty trees
                (acc, EmptyTree, EmptyTree) -> (EmptyTree, acc)

                -- If the first tree is empty, return the second tree as the accumulated remainder
                (EmptyTree, EmptyTree, tree2) -> (EmptyTree, tree2)

                -- If the second tree is non-empty and the first tree is empty, attach the second tree to the accumulated remainder
                (acc, EmptyTree, tree2) -> (attach acc tree2, EmptyTree)

                -- If the second tree is empty, put the tree as remainder
                (EmptyTree, tree1, EmptyTree) -> (EmptyTree, tree1)

                -- If the first tree is non-empty and the second tree is empty and we have remainder, attach the first tree to the accumulated remainder
                (acc, tree1, EmptyTree) -> (attach acc tree1, EmptyTree)

                -- If both trees are non-empty, attach the second tree to the first tree as the remainder
                (EmptyTree, tree1, tree2) -> (attach tree1 tree2, EmptyTree)

                -- If both trees are non-empty and we have remainder, attach the second tree to the first tree and let the remainder as it is
                (acc, tree1, tree2) -> (attach tree1 tree2, acc)

    -- If the resulting tree is empty, return the accumulated remainder
    -- Otherwise, concatenate the accumulated remainder with the resulting tree because it does not have to be left out
    in if fst res == EmptyTree then snd res else snd res ++ [fst res]
...
-}
merge :: (Ord p, Eq k) => BinomialHeap p k -> BinomialHeap p k -> BinomialHeap p k
...
-}
isolate :: a -> [a] -> [(a, [a])]
isolate a xs = eraseLast $ isolateHelper xs a []
  where
    isolateHelper :: [a] -> a -> [a] -> [(a, [a])]
    isolateHelper [] _ _ = []
    isolateHelper (x:xs) a prev = (x, prev ++ [a] ++ xs) : isolateHelper xs a (prev ++ [x])

eraseLast :: [(a, [a])] -> [(a, [a])]
eraseLast [] = []
eraseLast [(x, xs)] = [(x, init xs)]
eraseLast (p:ps) = p : eraseLast ps
...
-- deoarece putem evita efectuarea unor calcule inutile. În cazul în care 
-- heap-ul dat ca argument al funcției isolate este vid, nu este necesară 
-- efectuarea niciunei operații, astfel că evaluarea leneșă poate evita 
-- calculul inutil al expresiei.
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    show tree = showTree tree ""
        where
            showTree :: (Show p, Show k) => BinomialTree p k -> String -> String
            showTree EmptyTree _ = "*"
            showTree (Node prio key children) indent =
                show prio ++ " (" ++ show key ++ ")" ++
                concatMap (\x -> "\n  " ++ indent' ++ showTree x (indent' ++ "  ")) children
                where indent' = if null indent then "" else "" ++ indent
...
instance Functor (BinomialTree p) where
    ...
    fmap f tree = case tree of
            EmptyTree -> EmptyTree
            Node prio key children -> Node prio (f key) (map (fmap f) children)
...
instance Functor (BinomialHeap p) where
    ...
    fmap f heap = heap { trees = map (fmap f) (trees heap) }
